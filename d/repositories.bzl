...
def _d_repo_impl(repository_ctx):
    url = "https://github.com/someorg/someproject/releases/download/v{0}/d-{1}.zip".format(
        repository_ctx.attr.d_version,
        repository_ctx.attr.platform,
    )
    repository_ctx.download_and_extract(
        url = url,
        integrity = SDK_VERSIONS[repository_ctx.attr.d_version][repository_ctx.attr.platform],
    )
    build_content = """# Generated by d/repositories.bzl
load("@rules_d//d:toolchain.bzl", "d_toolchain")

d_toolchain(
    name = "d_toolchain",
    target_tool = select({
        "@bazel_tools//src/conditions:host_windows": "d_tool.exe",
        "//conditions:default": "d_tool",
    }),
)
"""
