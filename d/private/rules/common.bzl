...
def compilation_action(ctx, target_type = TARGET_TYPE.LIBRARY):
    ...
    library_names = depset(transitive = [d.library_names for d in d_deps])
    library_paths = depset(transitive = [d.library_paths for d in d_deps])
    ...
    if target_type in [TARGET_TYPE.BINARY, TARGET_TYPE.TEST]:
        ...
        args.add_all(library_names.to_list(), format_each = "-L-l%s")
        args.add_all(library_paths.to_list(), format_each = "-L-L%s")
    ...
    if target_type == TARGET_TYPE.LIBRARY:
        return [
            ...
            DInfo(
                ...
                library_names = depset(
                    [ctx.label.name] if target_type == TARGET_TYPE.LIBRARY and output else [],
                    transitive = [d.library_names for d in d_deps],
                ),
                library_paths = depset(
                    [output.dirname] if target_type == TARGET_TYPE.LIBRARY and output else [],
                    transitive = [d.library_paths for d in d_deps],
                ),
                string_import_paths = depset(
