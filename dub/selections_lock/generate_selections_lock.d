...
void unpack(Package package_)
{
    ...
    scope (failure)
    {
        package_.unpackDir.rmdirRecurse;
    }
    ...
    foreach (name, am; zip.directory)
    {
        ...
        if (!name.endsWith("/"))
            continue;
        buildNormalizedPath(package_.unpackDir, "..", name).mkdirRecurse;
    }
    // then expand files
    foreach (name, am; zip.directory)
    {
        if (name.endsWith("/"))
            continue;
        zip.expand(am);
        am.expandedData.toFile(buildNormalizedPath(package_.unpackDir, "..", name));
    }
}
...
string computeBuildFile(Package package_)
{
    ...
    if (!package_.unpackDir.exists)
        package_.unpack;

    auto result = execute([
        config.dubExecutable,
        "describe",
        "--root=%s".format(package_.unpackDir),
    ]);
